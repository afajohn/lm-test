...
export default function CategoryTable({entitiesData }: UseEntityTableProps) {
  const router = useRouter();
  const { 
    handleDeleteClick, deleteEntityId,
    closeDeleteModal, confirmDelete
    } = useEntityTable("categories");
  return (
    <div className="p-6 bg-white rounded-lg shadow-md">
      <div className="flex justify-between items-center mb-4">
        ...
        <Button onClick={() =>router.push("/afa-admin/categories/category-form?id=new-article")} className="bg-green-500">+ Add Category</Button>
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {entitiesData.entities.length === 0 ? (
                ...
                entitiesData.entities.map((category: Entity) => (
                  <TableRow key={category.id}>
                    <TableCell>{category.name}</TableCell>
                    <TableCell>{category.slug}</TableCell>
                    <TableCell>{category.author}</TableCell>
                    <TableCell><TimePosted dateString={category.date}/></TableCell>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                              <Button 
                                  ...
                                  onClick={() => router.push(`/afa-admin/categories/category-form?id=${category.id}`)}><Pencil /></Button>
                          </TooltipTrigger>
                          ...
                          <Tooltip>
                              <TooltipTrigger asChild>
                                  ...
                                  onClick={(e) => { e.stopPropagation(); handleDeleteClick(category.id ?? `${category.id}`); }}><Trash2 /> </Button>
                              </TooltipTrigger>
